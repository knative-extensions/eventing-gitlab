...
import (
	...
	"knative.dev/eventing-gitlab/pkg/client/clientset/versioned/scheme"
)

type BindingsV1alpha1Interface interface {
	RESTClient() rest.Interface
	GitLabBindingsGetter
}
...
func (c *BindingsV1alpha1Client) GitLabBindings(namespace string) GitLabBindingInterface {
	return newGitLabBindings(c, namespace)
}
...
// NewForConfigAndClient creates a new BindingsV1alpha1Client for the given config and http client.
// Note the http client provided takes precedence over the configured transport values.
func NewForConfigAndClient(c *rest.Config, h *http.Client) (*BindingsV1alpha1Client, error) {
	config := *c
	if err := setConfigDefaults(&config); err != nil {
		return nil, err
	}
